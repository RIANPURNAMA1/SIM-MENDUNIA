...
export const lmsApi = {
  ...
  courseDetail: (id: number, password?: string) => api.get(`/lms/courses/${id}`, password ? { headers: { 'X-Password-Course': password } } : undefined),
  lessonDetail: (id: number) => api.get(`/lms/lessons/${id}`),
  lessonPembahasan: (lessonId: number, paketId: number) => api.get(`/lms/lessons/${lessonId}/pakets/${paketId}/pembahasan`),
...
export const guruLmsApi = {
  ...
  syncKehadiran: () => api.post('/guru/lms-sync-kehadiran'),
  ...
  attachLessonPaket: (id: number, quizPaketId: number, isPembahasan?: boolean) => api.post(`/guru/lms-lessons/${id}/pakets`, isPembahasan === undefined ? { quiz_paket_id: quizPaketId } : { quiz_paket_id: quizPaketId, is_pembahasan: isPembahasan ? 1 : 0 }),
  setLessonPaketStatus: (id: number, paketId: number, status: string) => api.patch(`/guru/lms-lessons/${id}/pakets/${paketId}/status`, { status }),
  setLessonPaketPembahasan: (id: number, paketId: number, isPembahasan: boolean) => api.post(`/guru/lms-lessons/${id}/pakets/${paketId}/pembahasan`, { is_pembahasan: isPembahasan ? 1 : 0 }),
...
export const guruQuizApi = {
  ...
  results: (paketId: number, kelasSenseiId?: number, batchId?: number, level?: string) => api.get(`/guru/quiz/pakets/${paketId}/results`, { params: { kelas_sensei_id: kelasSenseiId, batch_id: batchId, level } }),
  monitor: (paketId: number, date?: string, kelasSenseiId?: number, batchId?: number, level?: string) => api.get(`/guru/quiz/pakets/${paketId}/monitor`, { params: { date, kelas_sensei_id: kelasSenseiId, batch_id: batchId, level } }),
  leaderboard: () => api.get('/guru/quiz/leaderboard'),
...
export const adminQuizApi = {
  ...
  storeQuestionsBulk: (paketId: number, data: Record<string, unknown>) => api.post(`/admin-cabang/quiz/pakets/${paketId}/questions-bulk`, data),
