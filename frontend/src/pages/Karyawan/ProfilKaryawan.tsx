...
export default function ProfilKaryawan() {
  ...
  async function loadData() {
    setLoading(true)
    try {
      const res = await karyawanApi.detail(user!.id)
    ...
    <div className="min-h-screen bg-[#F4F5F8] pb-24">
      ...
      <div className="px-4 pt-4 pb-4 space-y-4 max-w-lg mx-auto">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm font-bold text-[#14182B] mt-3">{karyawan?.name || '—'}</p>
          <p className="text-[11px] text-[#8B90A0] font-medium">{karyawan?.jabatan || '—'}</p>
        </div>
          ...
          <>
            <div className="bg-white rounded-xl border border-[#E5E7EF] overflow-hidden">
              <div className="px-5 pt-4 pb-2">
                <h3 className="text-[11px] font-bold tracking-[0.08em] text-[#4B5063] uppercase">Data Karyawan</h3>
              </div>
              <div className="px-5 pb-4">
                {infoRow('NIP', karyawan?.nip)}
                {infoRow('NIK', karyawan?.nik)}
                {infoRow('Email', karyawan?.email)}
                {infoRow('Jabatan', karyawan?.jabatan)}
                {infoRow('Divisi', karyawan?.divisi?.nama_divisi)}
                {infoRow('Status Kerja', karyawan?.status_kerja)}
                {infoRow('Tanggal Masuk', karyawan?.tanggal_masuk)}
                {infoRow('No. HP', karyawan?.no_hp)}
                {infoRow('Tempat Lahir', karyawan?.tempat_lahir)}
                {infoRow('Tanggal Lahir', karyawan?.tanggal_lahir)}
                {infoRow('Jenis Kelamin', karyawan?.jenis_kelamin === 'L' ? 'Laki-laki' : karyawan?.jenis_kelamin === 'P' ? 'Perempuan' : null)}
                {infoRow('Agama', karyawan?.agama)}
                {infoRow('Status Pernikahan', karyawan?.status_pernikahan)}
                {infoRow('Pendidikan Terakhir', karyawan?.pendidikan_terakhir)}
                {infoRow('Alamat', karyawan?.alamat)}
              </div>
            </div>
