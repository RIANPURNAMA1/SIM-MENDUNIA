...
export default function AdminCabangDataKandidat() {
  ...
  const [tambahForm, setTambahForm] = useState({
    nama: '', email: '', nik: '', telepon: '', batch_id: '',
  ...
  async function handleTambahSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await adminCabangApi.createKandidat(payload)
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        ...
        if (axErr.response?.data?.errors) {
          ...
          for (const [field, msgs] of Object.entries(errs)) {
            fieldErrors[field] = `${field}: ${msgs[0]}`
          }
  ...
  return (
    <div className="px-3 py-3 sm:px-6 sm:py-4">
      ...
      {/* Tambah Data Modal */}
      {showTambah && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 pt-10 pb-10" onClick={resetTambah}>
          <div className="w-full max-w-4xl rounded-xl bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              ...
              <button onClick={resetTambah} className="rounded-lg p-1.5 hover:bg-slate-100 transition">
            ...
            {!tambahSuccess && (
              <div className="flex items-center justify-end gap-3 border-t border-slate-200 px-6 py-3.5">
                <button type="button" onClick={resetTambah}
