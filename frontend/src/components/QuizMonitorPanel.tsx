...
import {
  CheckCircle2, X, RefreshCw, Pause, Play, ShieldAlert, Camera,
} from 'lucide-react'
...
interface QuizMonitorPanelProps {
  paketId: number
  title?: string | null
  onClose?: () => void
}

export default function QuizMonitorPanel({ paketId, title, onClose }: QuizMonitorPanelProps) {
  ...
  const fetchMonitor = useCallback(() => {
    ...
    guruQuizApi.monitor(paketId).then(res => {
      setData(res.data)
  ...
  return (
    <div className="mt-3 rounded-md border border-[#1c2026] bg-[#16181d] text-white overflow-hidden shadow-sm">
      {/* ── Header ── */}
      <div className="border-b border-white/10 px-4 pt-3.5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h4 className="text-xs font-bold text-white truncate">{title || `Paket #${paketId}`}</h4>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button onClick={() => setPaused(p => !p)}
              className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[10px] font-bold transition-colors ${paused ? 'bg-amber-500/20 text-amber-400' : 'bg-white/5 text-slate-300 hover:bg-white/10'}`}>
              {paused ? <Play size={12} /> : <Pause size={12} />}
              {paused ? 'Lanjut' : 'Jeda'}
            </button>
            <button onClick={fetchMonitor} disabled={fetchingRef.current}
              className="flex items-center gap-1.5 rounded-md bg-[#0E6187] px-2.5 py-1.5 text-[10px] font-bold text-white hover:bg-[#0a4d6b] transition-colors disabled:opacity-60">
              <RefreshCw size={12} className={fetchingRef.current ? 'animate-spin' : ''} />
              Segarkan
            </button>
            {onClose && (
              <button onClick={onClose}
                className="flex items-center justify-center w-7 h-7 rounded-md bg-white/5 text-slate-300 hover:bg-white/10 transition-colors"
                title="Tutup monitoring">
                <X size={13} />
              </button>
            )}
          </div>
      ...
      <div className="p-4">
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-8 text-xs font-medium text-slate-400">
            <span className="w-5 h-5 border-2 border-[#0E6187]/20 border-t-[#0E6187] rounded-full animate-spin" />
            Membuka monitoring...
          </div>
        ) : failed && !data ? (
          <div className="py-8 text-center">
            <ShieldAlert size={26} className="text-red-400 mx-auto mb-2" />
            <p className="text-xs font-bold text-white">Gagal memuat monitoring</p>
            <button onClick={() => { setLoading(true); fetchMonitor() }}
              className="mt-2 text-xs font-bold text-[#6fb3d8] hover:underline">
              Coba Lagi
            </button>
          </div>
        ) : attempts.length === 0 ? (
          <div className="rounded-md border border-dashed border-white/15 bg-white/[0.02] p-8 text-center">
            <CheckCircle2 size={24} className="text-slate-500 mx-auto mb-2.5" />
            <p className="text-xs font-bold text-slate-200">Belum ada kandidat mengerjakan</p>
            <p className="text-[10px] text-slate-500 font-medium mt-1">Kandidat yang mulai mengerjakan quiz ini akan muncul di sini secara otomatis.</p>
          </div>
        ) : (
      ...
      {/* ── Detail modal ── */}
      {showDetail && (
        <div className="fixed inset-0 z-[95] flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4" onClick={() => setShowDetail(false)}>
          <div className="bg-white w-full sm:max-w-2xl rounded-t-xl sm:rounded-xl max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-[#F0F1F5] sticky top-0 bg-white">
              ...
              <button onClick={() => setShowDetail(false)} className="w-8 h-8 flex items-center justify-center rounded-md bg-[#F4F5F8] hover:bg-[#E5E7EF]">
                ...
                <>
                  ...
                  {detail.attempt.webcam_photo && (
                    <div>
                      ...
                      <img src={detail.attempt.webcam_photo ?? undefined} alt="Webcam" className="w-full rounded-md border border-[#E5E7EF] max-h-52 object-cover" />
                    </div>
                  )}
                                ...
                                <div className="flex items-center gap-2 mt-3">
                                  <div>
                                    ...
                                    <input type="number" min={0} max={q.points}
                                      ...
                                      className="w-24 text-xs border border-[#E5E7EF] rounded-md px-3 py-2 focus:outline-none focus:border-[#0069b0] focus:ring-2 focus:ring-[#0069b0]/10"
                                      placeholder="-" />
                                  </div>
                                  <button type="button" disabled={savingGrade !== null || !grades[q.id]?.trim()}
                                    onClick={() => saveGrade(q.id)}
                                    className="self-end text-[11px] font-bold text-white bg-[#0069b0] px-3.5 py-2 rounded-md disabled:opacity-40 hover:bg-[#004d7a]">
