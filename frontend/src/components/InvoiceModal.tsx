...
import { X, Download, Printer, FileText, CheckCircle, Loader, ShieldCheck } from 'lucide-react'
import { toDataURL } from 'qrcode'
import { adminCabangApi, companyProfileApi } from '../services/api'
...
interface InvoiceModalProps {
  pendaftarId: number | null
  onClose: () => void
}

export default function InvoiceModal({ pendaftarId, onClose }: InvoiceModalProps) {
  ...
  useEffect(() => {
    ...
    setQrDataUrl('')
    Promise.all([
      adminCabangApi.invoice(pendaftarId),
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      pdf.save(`Invoice-${data?.no_invoice || pendaftarId}.pdf`)
    } catch (err) {
  ...
  return createPortal(
    <div
      className="invoice-modal-overlay fixed inset-0 z-[80] flex items-center justify-center bg-black/40 px-3 py-4 sm:py-6"
      onClick={onClose}
    >
      <div
        className="invoice-modal-shell flex w-full max-w-[210mm] max-h-[92vh] flex-col overflow-hidden rounded-xl bg-white shadow-xl"
      ...
      >
        ...
        <div className="no-print flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-[#00C0FF] px-3 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-[#00a8e0] disabled:opacity-60"
            >
        ...
        <style>{`
          ...
          @media print {
            ...
            body.invoice-modal-open #root { display: none !important; }
            .no-print { display: none !important; }
            .print-only { display: block !important; }
            .invoice-modal-overlay { position: static !important; background: transparent !important; padding: 0 !important; display: block !important; }
            .invoice-modal-shell { position: static !important; background: transparent !important; padding: 0 !important; overflow: visible !important; max-height: none !important; box-shadow: none !important; border: none !important; border-radius: 0 !important; }
            .invoice-modal-shell > .no-print { display: none !important; }
            .invoice-modal-shell > div:last-child { display: none !important; }
            .invoice-modal-shell > div:nth-child(2) { padding: 0 !important; background: white !important; }
...
function InvoiceContent({
  ...
  return (
    <div
      ref={invoiceRef}
      className="invoice-print relative mx-auto max-w-[210mm] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg print:shadow-none print:border-0 print:rounded-none"
    >
      {/* Header */}
